...
from openzetc.agents.backends.sandbox.paths import sandbox_workspace_agent_context_file
from openzetc.utils.logging_config import logger
from openzetc.utils.paths import WORKSPACE_AGENT_CONTEXT_FILES
...
DEFAULT_OPENZETC_SUMMARY_PROMPT = """你是对话上下文压缩助手。
...
class BaseContext:
    ...
    summary_prompt: str = field(
        default=DEFAULT_OPENZETC_SUMMARY_PROMPT,
        metadata={
            "name": "上下文摘要提示词",
...
async def resolve_agent_resource_options(
    ...
    *,
    ...
    if "tools" in fields_to_load:
        from openzetc.agents.toolkits.service import get_tool_metadata
    ...
    if "mcps" in fields_to_load:
        try:
            from openzetc.agents.mcp.service import get_accessible_enabled_mcp_servers

            servers = await get_accessible_enabled_mcp_servers(db, user)
        except ImportError:
            # 兼容仅注入旧 MCP 服务接口的离线扩展与单元测试。
            from openzetc.agents.mcp.service import get_all_mcp_servers
    ...
    if "skills" in fields_to_load:
        from openzetc.agents.skills.service import list_accessible_skills
...
async def prepare_agent_runtime_context(
    ...
    *,
    ...
    from openzetc.agents.backends.knowledge_base_backend import resolve_visible_knowledge_bases_for_context
    from openzetc.agents.middlewares.skills import resolve_runtime_skills_for_context
