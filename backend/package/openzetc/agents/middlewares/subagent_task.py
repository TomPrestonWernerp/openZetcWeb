...
from openzetc.repositories.agent_run_repository import TERMINAL_RUN_STATUSES
...
from openzetc.storage.postgres.models_business import Agent


def _subagent_run_service_module():
    from openzetc.services import subagent_run_service
...
TASK_TOOL_DESCRIPTION = """Launch a configured openZetc subagent to handle an isolated task.
...
SUBAGENT_START_DESCRIPTION = """Start a configured openZetc subagent asynchronously.
...
async def create_subagent_task_middleware(parent_context) -> OpenZetcSubAgentMiddleware | None:
    ...
    if not subagents:
        return None
    return OpenZetcSubAgentMiddleware(parent_context=parent_context, subagents=subagents)


class OpenZetcSubAgentMiddleware(AgentMiddleware[Any, ContextT, ResponseT]):
    ...
    def _build_task_tool(self, available_agents: str) -> StructuredTool:
        ...
        async def atask(
            ...
            try:
                from openzetc.services.agent_run_service import AgentRunWaitTimeout, await_agent_run_result
    ...
    def _build_async_subagent_tools(self, available_agents: str) -> list[StructuredTool]:
        ...
        async def asubagent_status(
            ...
            from openzetc.services.agent_run_service import get_agent_run_progress, get_agent_run_result
        ...
        async def asubagent_cancel(
            ...
            from openzetc.services.agent_run_service import request_cancel_agent_run
