"""openZetc adapter for DeepAgents conversation summarization middleware."""
...
from openzetc.agents.backends.composite import create_agent_composite_backend
...
_TOOL_RESULT_SAVED_MARKER = "openzetc_tool_result_saved"
_SUMMARY_BACKEND: ContextVar[Any | None] = ContextVar("openzetc_summary_backend", default=None)
_SUMMARY_SANITIZED_MESSAGES: ContextVar[dict[tuple[int, ...], list[AnyMessage]] | None] = ContextVar(
    "openzetc_summary_sanitized_messages",
    default=None,
)
_SUMMARY_COMPRESSION_STATE: ContextVar[dict[str, bool] | None] = ContextVar(
    "openzetc_summary_compression_state",
    default=None,
)
...
def _emit_compression(status: str, **extra: Any) -> None:
    try:
        ...
        return
    writer({"type": "openzetc.context_compression", "status": status, **extra})
...
class OpenZetcSummarizationMiddleware(SummarizationMiddleware):
    """DeepAgents summarization middleware with openZetc-specific tool-call sanitization."""
...
def create_summary_middleware(
    ...
    *,
    ...
    """Create DeepAgents summarization middleware using openZetc's virtual outputs root."""
    middleware_kwargs = {
        "model": model,
    ...
    middleware = OpenZetcSummarizationMiddleware(**middleware_kwargs)
