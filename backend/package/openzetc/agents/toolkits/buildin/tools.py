...
from openzetc.agents.toolkits.registry import ToolExtraMetadata, _all_tool_instances, _extra_registry, tool
from openzetc.utils import logger
from openzetc.utils.paths import (
...
from openzetc.utils.question_utils import normalize_questions
...
async def ocr_parse_file(file_path: str, runtime: ToolRuntime, ocr_engine: str | None = None) -> dict:
    ...
    from openzetc.agents.backends.sandbox.paths import virtual_path_for_thread_file
...
def _resolve_ocr_source_path(file_path: str, runtime: ToolRuntime) -> tuple[str, str, Path]:
    ...
    from openzetc.agents.backends.sandbox.paths import get_virtual_path_prefix, resolve_virtual_path
...
def _next_ocr_output_path(thread_id: str, source_path: Path) -> Path:
    ...
    from openzetc.agents.backends.sandbox.paths import sandbox_outputs_dir
