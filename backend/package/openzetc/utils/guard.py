...
class ContentGuard:
    def __init__(self, keywords_file: str = None):
        ...
        # 模型缓存由应用 lifespan 在数据库初始化后写入 Redis。这里必须延迟加载，
        # 否则首次部署或缓存键升级时会在 lifespan 运行前读取空缓存，导致 API 无法启动。
        self.llm_model = None
    ...
    async def check_with_llm(self, text: str) -> bool:
        ...
        if self.llm_model is None:
            model_spec = config.content_guard_llm_model
            if not model_spec:
                logger.warning("LLM content guard enabled but no model is configured")
                return False
            self.llm_model = select_model(model_spec=model_spec)
