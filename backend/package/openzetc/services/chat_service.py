...
from openzetc.agents.context import build_agent_input_context, normalize_agent_context_config
from openzetc.agents.state import AgentStatePayload
...
from openzetc.repositories.subagent_thread_repository import SubagentThreadRepository
from openzetc.services.conversation_service import serialize_attachment
from openzetc.services.input_message_service import AgentRunInputMessage
from openzetc.services.langfuse_service import (
    LangfuseRunContext,
    build_run_context,
    flush_langfuse,
    get_trace_info,
)
from openzetc.services.subagent_run_service import serialize_subagent_run_state
from openzetc.storage.postgres.manager import pg_manager
from openzetc.storage.postgres.models_business import Agent, User
from openzetc.utils.guard import content_guard
from openzetc.utils.logging_config import logger
from openzetc.utils.question_utils import (
    normalize_questions as _normalize_interrupt_questions,
)
from openzetc.utils.thread_utils import extract_thread_id as _metadata_thread_id
...
def _message_chunk_openzetc_events(
...
def _protocol_event_openzetc_event(
    event: dict[str, Any],
    *,
    message_id: str | None,
    thread_id: str | None,
...
def _context_compression_payload(payload: Any) -> dict | None:
    if isinstance(payload, dict) and payload.get("type") == "openzetc.context_compression":
        return payload
    return None
...
def _message_payload_openzetc_events(
    msg: Any,
    *,
    ...
    if isinstance(msg, dict) and isinstance(msg.get("event"), str):
        ...
        stream_event = _protocol_event_openzetc_event(
            msg,
            message_id=message_id,
            thread_id=thread_id,
            namespace=namespace,
    ...
    return _message_chunk_openzetc_events(
        msg_dict,
        message_id=message_id,
        thread_id=thread_id,
        namespace=namespace,
    ...
    *,
    ...
    try:
        ...
        async for mode, payload in _stream_agent_events(
            ...
            stream_events = _message_payload_openzetc_events(
                msg,
                metadata=metadata,
                namespace=namespace,
