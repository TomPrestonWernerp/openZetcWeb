...
from openzetc.agents.models import resolve_chat_model_spec
...
from openzetc.utils.hash_utils import hash_id
...
def _compact_run_event_envelope(envelope: dict) -> dict | None:
    ...
    if event_type == "metadata":
        return None
    if event_type == "custom" and isinstance(payload, dict) and payload.get("name") == "openzetc.agent_state":
...
async def get_active_run_by_thread(*, thread_id: str, current_uid: str, db: AsyncSession) -> dict:
    ...
    from openzetc.storage.postgres.models_business import AgentRun
