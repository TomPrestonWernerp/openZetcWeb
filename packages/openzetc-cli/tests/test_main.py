...
def test_remote_command_prints_version_and_remote_context_first(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openzetc_cli.main._store", lambda: store)
    monkeypatch.setattr("openzetc_cli.main.remote_ping", fake_remote_ping)

    result = CliRunner().invoke(app, ["remote", "ping"])

    assert result.exit_code == 0
    lines = [line.strip() for line in result.output.splitlines() if line.strip()]
    assert lines[:3] == [f"openZetc CLI {__version__}", "Remote: local https://example.com", "pong"]
