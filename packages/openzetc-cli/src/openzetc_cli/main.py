...
from openzetc_cli import __version__
from openzetc_cli.agent_eval import AgentEvalError, AgentEvalOptions, run_langfuse_agent_experiment
from openzetc_cli.client import ClientError
from openzetc_cli.commands import (
    CommandError,
    login_with_api_key,
    login_with_browser,
...
from openzetc_cli.config import ConfigError, ConfigStore
from openzetc_cli.kb_upload import DEFAULT_CONCURRENCY, MAX_CONCURRENCY, KbUploadError, KbUploadOptions, run_kb_upload

console = Console()
app = typer.Typer(help="openZetc command line client.", invoke_without_command=True)
remote_app = typer.Typer(help="Manage openZetc remotes.")
agent_app = typer.Typer(help="Run and manage openZetc agents.")
...
def _print_remote_context(store: ConfigStore, remote_name: str | None) -> None:
    ...
    console.print(f"openZetc CLI {__version__}")
...
def eval_agent(
    ...
    agent_slug: str = typer.Option(..., "--agent-slug", help="openZetc agent slug."),
    ...
    timeout_seconds: float = typer.Option(900, "--timeout-seconds", help="Per item openZetc API timeout."),
):
