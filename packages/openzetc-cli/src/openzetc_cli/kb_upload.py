...
def add_uploaded_documents(client: OpenZetcClient, kb_id: str, uploaded: list[UploadResult]) -> dict:
...
def _ensure_kb_upload_supported(client: OpenZetcClient) -> None:
    try:
...
def _load_kb_types(client: OpenZetcClient) -> dict:
...
def _resolve_database(client: OpenZetcClient, kb_id: str | None, kb_types: dict, console: Console) -> dict:
...
def _list_uploadable_databases(client: OpenZetcClient, kb_types: dict) -> list[dict]:
...
def _load_supported_extensions(client: OpenZetcClient) -> set[str]:
